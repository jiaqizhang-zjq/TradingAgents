...
*初阶研究员（Junior）*：
- **看多研究员** (`bull_researcher.py`): 预设看多立场，识别机会和增长潜力
- **看空研究员** (`bear_researcher.py`): 预设看空立场，识别风险和危险信号

*高级研究员（Senior/Master）— 无预设立场，独立判断*：
- **巴菲特研究员** (`buffett_researcher.py`): 价值投资视角（护城河、安全边际、FCF）
- **Cathie Wood 研究员** (`cathie_wood_researcher.py`): 颠覆性创新与成长股
- **Peter Lynch 研究员** (`peter_lynch_researcher.py`): GARP 成长合理价格策略
- **查理·芒格研究员** (`charlie_munger_researcher.py`): 逆向思维与多元心智模型
- **索罗斯研究员** (`soros_researcher.py`): 宏观对冲与反身性理论
- **达利欧研究员** (`dalio_researcher.py`): 全天候资产配置与经济周期
- **利弗莫尔研究员** (`livermore_researcher.py`): 趋势跟踪与关键点理论

*基础设施*：
- **基础研究员** (`base_researcher.py`): 抽象基类，统一所有研究员的创建逻辑
- **研究管理器** (`research_manager.py`): 组织 N-way round-robin 辩论，追踪历史胜率，综合结论

> 研究员通过 `RESEARCHER_REGISTRY`（定义在 `constants.py`）动态注册，默认选中 `["bull", "bear", "buffett"]`。
...
1. 初始化状态（ticker + date）
2. 分析师并行/顺序执行 → 生成报告
3. N-way round-robin 研究员辩论（默认 3 人 × 2 轮 = 6 次发言）→ 综合投资论点
4. 交易员提出交易 → 生成交易报告
...
**常量定义** (`tradingagents/constants.py`):
- 置信度常量：STRONG_CONFIDENCE (0.75), WEAK_CONFIDENCE (0.55), NEUTRAL_CONFIDENCE (0.65)
- 胜率常量：DEFAULT_BULL_WIN_RATE (0.52), DEFAULT_BEAR_WIN_RATE (0.48), DEFAULT_NEUTRAL_WIN_RATE (0.50)
- 重试策略：MAX_RETRY_ATTEMPTS (3), RETRY_DELAY_SECONDS (2)
- 缓存策略：CACHE_TTL_HOURS (24), MAX_CACHE_SIZE (1000)
- 辩论配置：MAX_DEBATE_ROUNDS (2), MAX_RISK_DISCUSS_ROUNDS (2)
- 数据验证：MIN_STOCK_DATA_DAYS (200), PREDICTION_THRESHOLD (0.02)
- 技术指标：RSI_OVERBOUGHT (70), RSI_OVERSOLD (30), CCI_PERIOD (20) 等
- **研究员注册表** (`RESEARCHER_REGISTRY`): 9 种研究员的动态注册字典（type, display_name, speaker_label, module, factory, default_win_rate）
- **默认研究员** (`DEFAULT_SELECTED_RESEARCHERS`): `["bull", "bear", "buffett"]`
...
2. Graph 流式执行:
   ...
   - Researchers (N-way round-robin 辩论，默认 Bull ↔ Bear ↔ Buffett × 2 轮)
...
**辩论机制**（N-way round-robin）:
```
研究员 A (如 Bull) → 提出论点
  ↓
研究员 B (如 Bear) → 反驳 + 提出论点
  ↓
研究员 C (如 Buffett) → 反驳 + 提出论点
  ↓
回到研究员 A → 反驳前面论点
  ↓
... 循环直到 count >= N * max_debate_rounds
  ↓
Research Manager → 综合所有研究员观点，给出最终判断
```

> 默认：3 人 × 2 轮 = 6 次发言 ≈ 25k tokens。研究员列表通过 `config["selected_researchers"]` 配置。
...
**添加新研究员**:
1. 在 `agents/researchers/` 创建新文件，继承 `base_researcher.py` 的模式
2. 实现 `create_xxx_researcher(llm, ...)` 工厂函数
3. 在 `constants.py` 的 `RESEARCHER_REGISTRY` 中注册
4. 通过 `config["selected_researchers"]` 选中即可参与辩论

**修改辩论逻辑**:
- 调整 `config["selected_researchers"]` 选择参与辩论的研究员
- 调整 `max_debate_rounds` 控制辩论轮数
- 修改 `conditional_logic.py` 中的 `should_continue_debate`（终止条件：`count >= N * max_debate_rounds`）
- 在 `researchers/` 修改研究员的提示词
...
**3. 双辩论系统**: 
- **投资辩论** (InvestDebateState): N-way round-robin（默认 Bull + Bear + Buffett）
...
### 重构 v0.3.0（2026-04）— 深度优化
- **自定义异常体系**: 15+ 异常类，覆盖 Data/LLM/State/Config/API/Agent/Cache/Database 域
- **异常精细化**: `raise Exception` 19→0, `except Exception` 80+→22 intentionally broad
- **安全升级**: `hashlib.md5` → `hashlib.sha256`
- **研究员系统扩展**: 新增 6 位大师研究员（charlie_munger, soros, dalio, livermore, cathie_wood, peter_lynch）
- **base_risk_debator**: `RiskDebatorConfig` 数据类 + `create_risk_debator()` 工厂函数
- **prediction_utils**: 通用预测提取，支持中英文双语
- **魔法数字常量化**: 50+ 处替换
- **重复代码消除**: `DataLoaderMixin`（-93行）、`DatabaseMixin`（-45行）
- **单元测试**: 178 个全部通过

### 重构 v0.2.0（2026-02）— 安全与性能
...
### 未来优化方向
- ✅ ~~P1-5: 数据管理器重构（单一职责拆分）~~ — 已完成
- ✅ ~~P1-6: 依赖注入（消除全局状态）~~ — 已完成（`core/container.py`）
- ✅ ~~P1-7: Agent工厂模式~~ — 已完成
- ✅ ~~P2-3: Pydantic配置管理~~ — 已完成
- ⬜ P2-5: Prompt模板文件化（`prompts/` 目录已创建但为空）
